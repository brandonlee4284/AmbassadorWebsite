...
#---------------------------------------------------------------------
def get_school_0(language_pod_dictionary, pod_number):
    school_0_count = 0
    for index, key in enumerate(language_pod_dictionary["Pod {0}".format(pod_number)]): 
...
def get_school_1(language_pod_dictionary, pod_number):
    school_1_count = 0
    for index, key in enumerate(language_pod_dictionary["Pod {0}".format(pod_number)]): 
...
def get_school_3(language_pod_dictionary, pod_number):
    school_3_count = 0
    for index, key in enumerate(language_pod_dictionary["Pod {0}".format(pod_number)]): 
...
def get_school_5(language_pod_dictionary, pod_number):
    school_5_count = 0
    for index, key in enumerate(language_pod_dictionary["Pod {0}".format(pod_number)]): 
...
def calculate_school_ratio(language_pod_dictionary, pod_number, name):
    ...
    school_0_ratio = f"School 0: {str(round(school_0_count/total_students,2))}\n" 
    school_1_ratio = f"School 1: {str(round(school_1_count/total_students,2))}\n" 
    school_3_ratio = f"School 3: {str(round(school_3_count/total_students,2))}\n" 
    school_5_ratio = f"School 5: {str(round(school_5_count/total_students,2))}"
...
def show_ratios():
    for x in range(len(english_pod_dictionary)):
        ...
        (str(calculate_school_ratio(english_pod_dictionary, x+1, "English")) + "\n\n")
    ...
    for x in range(len(spanish_pod_dictionary)):
        ...
        (str(calculate_school_ratio(spanish_pod_dictionary, x+len(english_pod_dictionary)+1, "Spanish")) + "\n\n")
    ...
    for x in range(len(other_pod_dictionary)):
        ...
        (str(calculate_school_ratio(other_pod_dictionary, x+len(spanish_pod_dictionary)+len(english_pod_dictionary)+1, "Other")) + "\n\n")
    ...
    for x in range(len(newly_add_pods_dict)):
        ...
        (str(calculate_school_ratio(newly_add_pods_dict, x+len(spanish_pod_dictionary)+len(english_pod_dictionary)+len(other_pod_dictionary)+1, "New")) + "\n\n")
...
#---------------------------------------------------------------------
def redistribute_schools(language_pod_dictionary, pod_number, name):
    ...
    old_standard_deviation = calculate_school_ratio(language_pod_dictionary, pod_number, name)

    school_0 = get_school_0(language_pod_dictionary, pod_number)
    school_1 = get_school_1(language_pod_dictionary, pod_number)
    school_3 = get_school_3(language_pod_dictionary, pod_number)
    school_5 = get_school_5(language_pod_dictionary, pod_number)
    ...
    new_standard_deviation = calculate_school_ratio(language_pod_dictionary, pod_number, name)
    return new_standard_deviation

def redistribute_schools_female(language_pod_dictionary, pod_number, name):
    # find which school is the most and least dominant
    # add the least dominant from (language_pod_dictionary+1) and remove most dominant from (language_pod_dictionary)
    # have to add/remove with same gender
    old_standard_deviation = calculate_school_ratio(language_pod_dictionary, pod_number, name)
...
def fix_school_ratio_by_males(language_pod_dictionary, name):
    if name == "English":
        pod_number = 1
    if name == "Spanish":
        pod_number = len(english_pod_dictionary) + 1
    if name == "Other":
        pod_number = len(english_pod_dictionary) + len(spanish_pod_dictionary) + 1

    standard_deviation = calculate_school_ratio(language_pod_dictionary, pod_number, name)
    i = 0
    while i != len(language_pod_dictionary)-1:
        standard_deviation = calculate_school_ratio(language_pod_dictionary, pod_number, name)
        while True:
            if (standard_deviation > 0.5):
                standard_deviation = redistribute_schools(language_pod_dictionary, pod_number, name)
                if redistribute_schools(language_pod_dictionary, pod_number, name) == False:
                    print("skipped...")
                    break
            else:
                break
...
def fix_school_ratio_by_females(language_pod_dictionary, name):
    if name == "English":
        pod_number = 1
    if name == "Spanish":
        pod_number = len(english_pod_dictionary) + 1
    ...
    while i != len(language_pod_dictionary)-1:
        ...
        while True:
            if (standard_deviation > 1.5):
                standard_deviation = redistribute_schools_female(language_pod_dictionary, pod_number, name)
                if redistribute_schools_female(language_pod_dictionary, pod_number, name) == False:
                    print("skipped...")
                    break
            else:
                break
...
fix_school_ratio_by_males(english_pod_dictionary, "English")
fix_school_ratio_by_males(spanish_pod_dictionary, "Spanish")
fix_school_ratio_by_males(other_pod_dictionary, "Other")
fix_school_ratio_by_females(english_pod_dictionary, "English")
fix_school_ratio_by_females(spanish_pod_dictionary, "Spanish")
fix_school_ratio_by_females(other_pod_dictionary, "Other")
#show_ratios()



def final_redistribute_male(language_pod_dictionary, pod_number, name):
    ...
    calculate_school_ratio(language_pod_dictionary, pod_number, name)
    ...
    while i != len(language_pod_dictionary):
        std = calculate_school_ratio(language_pod_dictionary,pod_number,name)
        if std == 0:
            for index, key in enumerate(language_pod_dictionary["Pod {0}".format(pod_number)]): # call by index
                if language_pod_dictionary["Pod {0}".format(pod_number)][str(key)]["Last Schl"] == dominant_school and language_pod_dictionary["Pod {0}".format(pod_number)][str(key)]["Gender"] == "M":
                    dominant_student = dict(itertools.islice(language_pod_dictionary["Pod {0}".format(pod_number)].items(), index, index+1))
...
def final_redistribute_female(language_pod_dictionary, pod_number, name):
    # search for a pod that has a standard deviation of 0
    # determine which school is most/least dominant in the pod that has the highest standard deviation
    # take the least dominant school from the pod with 0 std and add to the one with higher std
    calculate_school_ratio(language_pod_dictionary, pod_number, name)
...
def finalize_school_redistributions_female(language_pod_dictionary, name):
    if name == "English":
        pod_number = 1
    if name == "Spanish":
        pod_number = len(english_pod_dictionary) + 1
    ...
    while i != len(language_pod_dictionary):
        ...
        while True:
            if (standard_deviation > 1.3):
                standard_deviation = final_redistribute_female(language_pod_dictionary, pod_number, name)
                if final_redistribute_female(language_pod_dictionary, pod_number, name) == False:
                    print("skipped...")
                    break
            else:
                break
...
def finalize_school_redistributions_male(language_pod_dictionary, name):
    if name == "English":
        pod_number = 1
    if name == "Spanish":
        pod_number = len(english_pod_dictionary) + 1
    ...
    while i != len(language_pod_dictionary):
        ...
        while True:
            if (standard_deviation > 1.3):
                standard_deviation = final_redistribute_male(language_pod_dictionary, pod_number, name)
                if final_redistribute_male(language_pod_dictionary, pod_number, name) == False:
                    print("skipped...")
                    break
            else:
                break
...
finalize_school_redistributions_female(english_pod_dictionary, "English")
finalize_school_redistributions_female(spanish_pod_dictionary, "Spanish")
finalize_school_redistributions_female(other_pod_dictionary, "Other")
finalize_school_redistributions_male(english_pod_dictionary, "English")
finalize_school_redistributions_male(spanish_pod_dictionary, "Spanish")
finalize_school_redistributions_male(other_pod_dictionary, "Other")


def count_std_above_threshold(language_pod_dictionary, name):
    count = 0
    if name == "English":
        pod_number = 1
    if name == "Spanish":
...
english_over_threshold = count_std_above_threshold(english_pod_dictionary, "English")
spanish_over_threshold = count_std_above_threshold(spanish_pod_dictionary, "Spanish")
other_over_threshold = count_std_above_threshold(other_pod_dictionary, "Other")
if len(newly_add_pods_dict) != 0:
    new_over_threshold = count_std_above_threshold(newly_add_pods_dict, "New")
#show_ratios()
